...
public abstract class JPAAbstractQuery<R> {
    ...
    private String applyIsolationFilter(
        ...
        if (rootProcessIdColumn != null) {
            isolationPredicate =
                    "("
                            + "EXISTS (SELECT 1 FROM _allowed_processes ap WHERE ap.processId = " + processIdColumn
                            + " AND (" + processVersionColumn + " IS NULL OR ap.processVersion = " + processVersionColumn + "))"
                            + " OR "
                            + "EXISTS (SELECT 1 FROM _allowed_processes ap WHERE ap.processId = " + rootProcessIdColumn
                            + " AND (" + rootProcessVersionColumn + " IS NULL OR ap.processVersion = " + rootProcessVersionColumn + "))"
                            + ")";
        } else {
            isolationPredicate =
                    "EXISTS (SELECT 1 FROM _allowed_processes ap WHERE ap.processId = " + processIdColumn
