...
/**
 * Unit-level data-isolation test for {@link JPAAbstractQuery}.
 *
 * Boots a real Hibernate+H2 persistence unit (no Quarkus/Spring container),
 * inserts rows directly via EntityManager and exercises the isolation
 * methods through a minimal concrete subclass.
 *
 * Three dimensions are verified for both process-instance and job queries:
 * <ol>
 * <li>No isolation (processes = empty) → all rows visible</li>
 * <li>Isolation enabled, correct service → only matching rows visible</li>
 * <li>Isolation enabled, correct id but wrong version → row not visible</li>
 * <li>Isolation enabled, subprocess rooted at allowed process → row visible via root_process_id</li>
 * <li>Isolation enabled, foreign service → row not visible</li>
 * </ol>
 */
@TestInstance(TestInstance.Lifecycle.PER_CLASS)
class JPAAbstractQueryDataIsolationTest {

    // ── inner concrete subclass that exposes the protected helpers ────────────
    ...
    // ── PROCESS INSTANCE ISOLATION TESTS ─────────────────────────────────────
    ...
    void testProcessInstance_isolationEnabled_onlyMatchingServiceVisible() {
        ...
        // column index 3 = processId in GetAllProcessInstancesState SELECT
